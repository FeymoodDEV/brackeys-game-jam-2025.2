...
@export var wander_radius: float = 150.0  # max distance from spawn
...
func do_chase(delta: float) -> void:
	...
	if is_blocked(velocity.normalized()):
		pick_new_direction()
	else:
		strafe_timer -= delta
		if strafe_timer <= 0.0 or strafe_dir == Vector2.ZERO:
